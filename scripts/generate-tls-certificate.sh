...
generate() {
    ...
    openssl x509 -req -extfile <(printf "subjectAltName=DNS:guac-provider.${GATEKEEPER_NAMESPACE}") -days 1 -in server.csr -CA ca.crt -CAkey ca.key -CAcreateserial -out server.crt
}
